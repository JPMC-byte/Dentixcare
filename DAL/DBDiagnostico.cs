...
namespace DAL
{
    public class DBDiagnostico : ConexionOracle
    {
        ...
        public string guardarDato(Diagnostico diagnostico)
        {
        ...
        public List<Diagnostico> obtenerTodos()
        {
            ...
            string query = "SELECT * FROM DIAGNOSTICO ORDER BY TO_NUMBER(SUBSTR(ID_DIAGNOSTICO, 2))";
        ...
        {
            return obtenerTodos().FirstOrDefault<Diagnostico>(x => x.Codigo == codigo);
        }
        public List<Diagnostico> cargarPorCedula(string codigo)
        {
            return obtenerTodos().Where(diag => diag.CedulaPaciente == codigo).ToList();
        }
        public List<Diagnostico> cargarPorFecha(DateTime fecha)
        {
            return obtenerTodos().Where(diag => diag.Fecha_Diagnostico.Date == fecha.Date).ToList();
        }
        public List<Diagnostico> cargarPorFiltros(DateTime fecha, string cedulaPaciente)
        {
            List<Diagnostico> Diagnosticos = obtenerTodos();
        ...
        public string actualizarDato(Diagnostico diagnostico)
        {
