...
namespace GUI
{
    public partial class FrmSeleccionTratamiento : Form
    {
        ...
        public FrmSeleccionTratamiento(Diagnostico diagnostico, Persona persona)
        {
            InitializeComponent();
        ...
        {
            if(!Verificar()) { return; }
            if (ConfirmarAsignacion() && ValidarTratamientoAsignado()) 
                ConfirmarSeleccion();
        }
        ...
        void ConfirmarSeleccion()
        {
            ...
            if (!ValidarExistenteFactura())
            {
                ...
                factura.Cambio = 0;
                factura.CedulaPaciente = UsuarioActual.Cedula;
                servisFac.guardar(factura);

                servistrat.actualizarForaneaDiagnostico(tratamiento, diagnosticoSeleccionado.Codigo);
                servistrat.actualizarForaneaFactura(tratamiento,factura.ID_Factura);
            ...
            else
            {
                ...
                Factura factura = servisFac.obtenerPorCodigo(CodigoFactura);
