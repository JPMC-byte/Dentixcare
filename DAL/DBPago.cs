...
namespace DAL
{
    ...
    {
        public string guardarDato(Pago pago)
        {
            try
            {
        ...
        public List<Pago> obtenerTodos()
        {
            ...
            try
            {
                string query = "SELECT * FROM PAGO ORDER BY TO_NUMBER(SUBSTR(ID_PAGO, 3))";
                AbrirConexion();
        ...
        public List<Pago> cargarPorCodigoFactura(string codigo)
        {
            return obtenerTodos().Where(pago => pago.CodigoFactura == codigo).ToList();
        }
        public Pago obtenerPorCodigo(string id)
        {
            return obtenerTodos().FirstOrDefault<Pago>(x => x.ID_Pago == id);
        }
    }
}
