...
namespace DAL
{
    public class DBOrtodoncista : ConexionOracle
    {
        ...
        public string guardarDato(Ortodoncista ortodoncista)
        {
        ...
        {
            listaOrtodoncistas = obtenerTodos();
            return listaOrtodoncistas.Find(x => x.Cedula == ID);
        }

        //public List<Ortodoncista> cargarPorPrimerNombre(string PrimerNombre)
        //{
        //    List<Ortodoncista> ortodoncistas = new List<Ortodoncista>();
        //    try
        //    {
        //        string query = "SELECT * FROM ORTODONCISTA WHERE PRIMER_NOMBRE = :PrimerNombre";

        //        abrirConexion();

        //        using (OracleCommand command = new OracleCommand(query, conexion))
        //        {
        //            command.Parameters.Add(new OracleParameter("PrimerNombre", PrimerNombre)); 
        //            using (OracleDataReader reader = command.ExecuteReader())
        //            {
        //                while (reader.Read())
        //                {
        //                    ortodoncistas.Add(Map(reader));
        //                }
        //            }
        //        }
        //    }
        ...
        //public List<Ortodoncista> obtenerPorNombre(string Name)
        //{
        //    return cargarPorPrimerNombre(Name).Where(PrimerN => PrimerN.PrimerNombre == Name).ToList();
        //}

        public List<Ortodoncista> obtenerPorNombre(string Name)
        {
            return obtenerTodos().Where(PrimerN => PrimerN.PrimerNombre == Name).ToList();
        }

        public Ortodoncista obtenerPorUsuario(string ID, string contrasena)
        {
            listaOrtodoncistas = obtenerTodos();
